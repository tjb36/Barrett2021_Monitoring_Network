...
IPAddress IP_OYLab_Dev01(169, 254, 128, 115);
char recvdBuffer[10];
int recvdPacketSize;
...
void setup() {
  ...
  Ethernet.begin(mac, IP_OYLab_Dev01);
  Udp.begin(localPort);
...
void loop() {
  ...
  seedPower = (seedOutputVal) / 1.0;
  BoosTAOutputVal = analogRead(BoosTAOutput_Read);
  BoosTAPower = (BoosTAOutputVal) / 752.0 *2680.0 ;
  RepumpOutputVal = analogRead(RepumpOutput_Read);
  RepumpPower = (RepumpOutputVal) / 706.0 * 61.0;
